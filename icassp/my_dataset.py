...
class DUTSDataset(data.Dataset):
    def __init__(self, root: str, train: bool = True, transforms=None):
        ...
        if train:
            self.image_root = os.path.join(root, "dataset", "images","train")
            self.mask_root = os.path.join(root, "dataset", "maps","train")
        else:
            self.image_root = os.path.join(root, "dataset", "images","test")
            self.mask_root = os.path.join(root, "dataset", "maps","test")
